    ...
    <>
      <div className="card-contenttable">
        <div className="table-image-container">
          ...
          <div className="stars">
            {[...Array(5)].map((_, i) =>
              i < tableData.rating ? <FaStar key={i} /> : <FaRegStar key={i} />
            )}
          </div>
        </div>
        <div className="table-details">
          ...
          <p>
            <FaClock /> Horário: {formattedTime}
          </p>
          <p>
            <FaCalendar /> Data: {tableData.date}
          </p>
          <p>
            <FaDollarSign /> Valor: R$ {tableData.price}
          </p>
          <p>Descrição: {tableData.description}</p>
        </div>
        <Link to={`/dmesas/${tableData.id}`} className="button">
          Ver mesa
        </Link>
      </div>
    </>
